import React from "react";
import "./Login.css";

function LoginBG() {
  return (
    <div className="grid-container">
      <img src="https://songsearch-2916.kxcdn.com/static/albums/2020/02/26/18/673387_300x300.jpg" />
      <img src="https://upload.wikimedia.org/wikipedia/en/3/36/21_Savage_%E2%80%93_I_Am_Greater_Than_I_Was.png" />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/2/2a/2014ForestHillsDrive.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/1/1f/Eternal_Atake_Lil_Uzi_Vert.jpg"
        width="300px"
        height="300px"
      />

      <img
        src="https://upload.wikimedia.org/wikipedia/en/d/d3/JColeKOD.jpg"
        width="300px"
        height="300px"
      />

      <img
        src="https://images-na.ssl-images-amazon.com/images/I/81sKEPIbmxL._SL1500_.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/9/9e/BobbyTarantino2.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://images.complex.com/complex/image/upload/c_fill,dpr_auto,f_auto,fl_lossy,g_face,q_auto,w_1280/hcjrqlvc6dfhpjxob9nt.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/b/bb/Jay-Z_-_The_Blueprint_3.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/9/9b/Lil_Uzi_Vert_vs_the_World_2_cover_art.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/f/f3/Trench_Twenty_One_Pilots.png"
        width="300px"
        height="300px"
      />
      <img
        src="https://pyxis.nymag.com/v1/imgs/6d7/008/b20deeaf26fe2cdf0796d1e0ab4d5473bd-05-starboy.rsquare.w700.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://images-na.ssl-images-amazon.com/images/I/71GBsl4AoqL._SL1400_.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://i1.sndcdn.com/artworks-000516309615-7s38hc-t500x500.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://images.squarespace-cdn.com/content/v1/53b6eb62e4b06e0feb2d8e86/1594942678203-T2DZHNP3HW0BDATHQB5Z/ke17ZwdGBToddI8pDm48kLPswmMOqQZ9-Q6KHLjvbpZ7gQa3H78H3Y0txjaiv_0fDoOvxcdMmMKkDsyUqMSsMWxHk725yiiHCCLfrh8O1z5QPOohDIaIeljMHgDF5CVlOqpeNLcJ80NK65_fV7S1UTcpTqfU-ZEsztPyQLxhSSK-PhJjRDDFQG0l3_ZnmWi1QjT9byXZM3ISxo3y1NRptg/SamSpratt_NoPressure_Cover_Final+copy.jpg?format=1500w"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/f/f1/808s_%26_Heartbreak.png"
        width="300px"
        height="300px"
      />
      <img
        src="data:image/jpeg;base64,"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/7/70/Graduation_%28album%29.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/f/f6/Kendrick_Lamar_-_To_Pimp_a_Butterfly.png"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/c/c9/Logic_-_Bobby_Tarantino_%28album_cover%29.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/6/65/Luv_Is_Rage_2_cover.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://thesource.com/wp-content/uploads/2019/11/mbdtf.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/c/c6/Die_Lit_by_Playboi_Carti.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/7/7d/Blurryface_by_Twenty_One_Pilots.png"
        width="300px"
        height="300px"
      />
      <img
        src="https://images-na.ssl-images-amazon.com/images/I/91dkpgT92CL._SL1500_.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://i.scdn.co/image/ab67616d0000b27310e6745bb2f179dd3616b85f"
        width="300px"
        height="300px"
      />
      <img
        src="https://images-na.ssl-images-amazon.com/images/I/511Z66tpj-L._SY355_.jpg"
        width="300px"
        height="300px"
      />
      <img
        src="https://upload.wikimedia.org/wikipedia/en/5/51/Kendrick_Lamar_-_Damn.png"
        width="300px"
        height="300px"
      />
    </div>
  );
}

export default LoginBG;
